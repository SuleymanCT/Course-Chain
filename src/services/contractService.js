...
export const getTokenIdForCourse = async (courseId) => {
  try {
    const tokenId = await marketplaceContract.getTokenIdForCourse(courseId);
    return tokenId.toString();
  } catch (error) {
    console.error(`Error fetching Token ID for Course ID ${courseId}:`, error.message);
    return null;
  }
};
