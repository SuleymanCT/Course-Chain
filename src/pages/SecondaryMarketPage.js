...
const SecondaryMarketPage = () => {
  ...
  const handleListForSale = async () => {
    ...
    try {
      console.log(`Attempting to list Token ID: ${tokenId} for ${price} ETH`);
  ...
  const handleBuyNFT = async (tokenId, price) => {
    try {
      console.log(`Attempting to buy NFT with ID: ${tokenId} for ${price} ETH`);
      await buyNFT(tokenId);
      alert(`NFT with ID ${tokenId} successfully purchased!`);
      window.location.reload();
    } catch (error) {
